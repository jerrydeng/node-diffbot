...
var Diffbot = (exports.Diffbot = function(token) {
...
Diffbot.prototype.article = function(options, callback) {
  ...
  var diffbot_url =
    "http://api.diffbot.com/v3/article?token=" +
    this.token +
    "&url=" +
    encodeURIComponent(options.uri);
    ...
    if (error) {
      ...
      try {
        ...
        if (options.singlePage) {
          if (
            parsedJSON.objects !== undefined &&
            parsedJSON.objects[0] !== undefined
          ) {
...
Diffbot.prototype.frontpage = function(options, callback) {
  ...
  var diffbot_url =
    "http://api.diffbot.com/v3/frontpage?token=" +
    this.token +
    "&url=" +
    encodeURIComponent(options.uri) +
    "&format=json";
...
Diffbot.prototype.product = function(options, callback) {
  ...
  var diffbot_url =
    "http://api.diffbot.com/v3/product?token=" +
    this.token +
    "&url=" +
    encodeURIComponent(options.uri) +
    "&proxy";
...
Diffbot.prototype.analyze = function(options, callback) {
  ...
  request({ url: diffbot_url }, function(error, response, body) {
    if (error) {
      return callback(error, undefined);
    } else {
      try {
